...
| `agent/src/gateway/server.ts` | When `use_pi` or `config.agentUsePi`, call `runAgentTurnWithPi`; 503 if Pi requested but not available |
...
When **AGENT_USE_PI=1** or the request body has **use_pi: true**, the gateway runs the turn via the **Pi coding-agent** runtime instead of the default Sulala loop. This is an **additive** path: the default path is unchanged.

- **Config:** `agentUsePi` (env `AGENT_USE_PI`, default false). **Request:** `POST /api/agent/sessions/:id/messages` with body `{ "message": "...", "use_pi": true }`.
